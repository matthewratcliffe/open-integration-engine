#!/usr/bin/env bash
#
# Builds the Cluster engine extension into dist/cluster-<version>.zip.
#
...
#
# Nothing is bundled. The extension needs the engine's own controllers and, for
# the one job no controller covers -- counting queued messages left behind by a
# node that is gone -- MyBatis, which the engine already loads. So there are no
# third-party jars to verify and nothing extra in the engine's JVM.
#
# Environment:
...
########################################################################
package() {
    local stage="${BUILD}/stage/cluster"
    rm -rf "${BUILD}/stage"
    ...
    MSYS_NO_PATHCONV=1 docker run --rm -v "${HERE}:/p" -w /p "$JDK_IMAGE" \
        sh -c 'cd build/classes && jar cf ../stage/cluster/libs/cluster-server.jar .'
...
JAVA
        java /tmp/ValidateXml.java build/stage/cluster/plugin.xml
    ...
    local zip="${DIST}/cluster-${PLUGIN_VERSION}.zip"
    rm -f "$zip"
    MSYS_NO_PATHCONV=1 docker run --rm -v "${HERE}:/p" -w /p "$JDK_IMAGE" \
        sh -c "cd build/stage && jar cfM ../../dist/cluster-${PLUGIN_VERSION}.zip cluster"
...
Install it by dropping the zip in extensions/ and restarting:
    ...
    cp $(printf '%s' "${DIST}/cluster-${PLUGIN_VERSION}.zip" | sed "s|${ROOT}/||") extensions/
    docker compose up -d --force-recreate engine
    ./scripts/oie-check-extensions.sh "Cluster"

It does nothing until a node is told it is part of a cluster:
